...
impl Timeline {
    pub fn new(name: impl Into<String>) -> Self {
        Self {
            ...
            left_time: NaiveTime::parse_from_str("00:00:00.0", "%H:%M:%S%.f").expect("WHy!!!!"),
            playback_speed: 1.0,
            tracks: vec![
...
impl Timeline {
    pub fn draw(&mut self, ui: &imgui::Ui, dt: f32) {
        ...
        if let Some(_token) = ui
            ...
            .begin()
        {
            let fdl = ui.get_window_draw_list();
            let mut top_left: Point = ui.window_pos().into();
            top_left += ui.cursor_pos().into();

            //Prevent drag
            let cursor = ui.cursor_pos();
            // ui.invisible_button("##timeline_bg", ui.content_region_max());
            ui.button_with_size("##timeline_bg", ui.content_region_max());
                ...
                .begin()
            {
                ...
                if let Some(_table) = ui.begin_table_with_flags(
                    ...
                    if ui.is_window_focused() && ui.is_key_index_pressed_no_repeat(ImGuiKey_Space) {
                    ...
                    ui.set_cursor_pos([
                        (max.x - size.x) / 2.0,
                        ui.cursor_pos()[1]
                            + (FIRST_HEIGHT - ui.text_line_height_with_spacing()) / 2.0,
                    ]);
                    ui.text(time);
                        ...
                        .begin()
                    {
                        ...
                        if ui.is_window_focused() {
                            if ui.is_key_index_pressed_no_repeat(ImGuiKey_Space) {
                                self.playing = !self.playing;
                            }
                            ...
                            || {
                                ...
                                while x <= width {
                                    ...
                                    for i in 1..10 {
                                        fdl.add_line(
                                            [
                                                top_left.x + x + second_width / 10.0 * i as f32,
                                    ...
                                    fdl.add_text(
                                        ...
                                        [1.0; 3],
                                        (self.left_time.overflowing_add_signed(
                        ...
                        let track_head_center = top_left.x
                            + ((self.time - self.left_time).num_milliseconds() as f32 / 1000.0)
                                * second_width;

                        // fdl.add_line(
                        //     [track_head_center, top_left.y],
